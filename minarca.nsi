...
;Configuration
  ...
  ; Define icon
  !define MUI_ICON "minarca_client/ui/images/minarca.ico"
  !define MUI_UNICON "minarca_client/ui/images/minarca.ico"
...
Section "Installation of $(DisplayName)" SecAppFiles
  ...
  WriteRegStr HKLM "${REG_UNINSTALL}" "DisplayIcon" "$INSTDIR\minarca_client\ui\images\minarca.ico"
  ...
  CreateShortCut "$DESKTOP\$(DisplayName).lnk" "$INSTDIR\minarcaw.exe" "" "$INSTDIR\minarca_client\ui\images\minarca.ico" 0
  CreateShortCut "$SMPROGRAMS\$(DisplayName)\${AppName}.lnk" "$INSTDIR\minarcaw.exe" "" "$INSTDIR\minarca_client\ui\images\minarca.ico" 0
