...
public class Keygen {
	 ...
	 */
	private static byte[] encodePrivateKeyForPutty(
			RSAPrivateCrtKey rsaPrivateKey) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void toPrivatePEM(RSAPrivateKey privateKey, Writer writer)
			throws IOException {
	...
	public static void toPrivatePuttyKey(KeyPair pair, String comment,
			FileWriter writer) throws IOException, NoSuchAlgorithmException,
			InvalidKeyException {
		...
		byte[] publicKeyEncoded = encodePublicKey((RSAPublicKey) pair
				.getPublic());
		String publicKeyBase64 = encodeBase64(publicKeyEncoded);
		byte[] privateKeyEncoded = encodePrivateKeyForPutty((RSAPrivateCrtKey) pair
				.getPrivate());
		...
		for (int i = 0; i < publicKeyBase64.length(); i += 64) {
			buf.append(publicKeyBase64.substring(i,
					Math.min(publicKeyBase64.length(), i + 64)));
			buf.append(CRLF);
		}
		buf.append("Private-Lines: " + countLines(privateKeyBase64, 64) + CRLF);
		for (int i = 0; i < privateKeyBase64.length(); i += 64) {
			buf.append(privateKeyBase64.substring(i,
					Math.min(privateKeyBase64.length(), i + 64)));
			buf.append(CRLF);
		}
