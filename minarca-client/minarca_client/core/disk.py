...
def _get_device_info(device):
    if IS_WINDOWS:
        # On Windows will gather detailed information using WMI.
        # Lookup Logical Disk e.g.: "C:"
        logical_disks = c.Win32_LogicalDisk(DeviceId=device.strip('\\'))
        if not logical_disks:
            # Ignore virtual disk
            return None
        disk_part = logical_disks[0].associators(wmi_result_class="Win32_DiskPartition")
        if not disk_part:
            # Ignore virtual disk
            return None
        disk_drive = disk_part[0].associators(wmi_result_class="Win32_DiskDrive")
        if not disk_drive:
            # Ignore virtual disk
            return None
        # MediaType could be an integer or a string.
        removable = disk_drive.MediaType in [11, 'Removable Media']
        return DeviceInfo(disk_drive[0].Caption, removable)
    elif IS_MAC:
        # On MacOS, will get information from diskutil commandline
        # Lookup device instead of partition.
        data = _diskutil_parse(subprocess.check_output(["diskutil", "info", device], env={'LANG': 'en_US'}, text=True))
        ...
        if disk_device != device:
            data = _diskutil_parse(
                subprocess.check_output(["diskutil", "info", disk_device], env={'LANG': 'en_US'}, text=True)
            )
...
def list_disks(removable=False):
    ...
    for part in parts:
        if IS_LINUX and part.mountpoint == '/':
