...
<BackupCard>:
    ...
    MDList:
        ...
        CListItem:
            ...
            md_bg_color: app.warning_20 if root.is_running else app.secondary_50
        ...
        CListItem:
            ...
            md_bg_color: app.white if root.last_backup_success else app.warning_20

            CListItemText:
                text:  root.last_backup_text

            MDListItemTrailingIcon:
                icon: "chevron-right" if root.last_backup_success else "alert-circle"
                icon_color: app.dark if root.last_backup_success else app.warning
        ...
        CListItem:
            ...
            md_bg_color: app.white if root.test_connection_success else app.warning_20
            ...
            MDListItemTrailingIcon:
                ...
                icon_color: app.dark if root.test_connection_success else app.warning
...
class BackupCard(CCard):
    ...
    async def _get_disk_usage(self, instance):
        ...
        try:
            value = await asyncio.get_event_loop().run_in_executor(None, instance.get_disk_usage)
        except Exception:
            # Network issue or dis not connected.
            logger.warn('problem while retrieving disk usage', exc_info=1)
            value = None
        # Update state.
        if value:
    ...
    def on_instance(self, widget, instance):
        ...
        self._status_task = asyncio.get_event_loop().create_task(self._watch_status(instance))
        self._settings_task = asyncio.get_event_loop().create_task(self._watch_settings(instance))
        self._disk_usage_task = asyncio.get_event_loop().create_task(self._get_disk_usage(instance))
    ...
    def stop_start_text(self):
        if self.in_transition:
            ...
            return _('Stop running backup') if self.is_running else _('Start Backup now')
    ...
    def toggle_pause(self, widget, value):
        ...
        self._pause_task = asyncio.get_event_loop().create_task(_pause())
