...
class PatternsViewTest(unittest.TestCase):
    ...
    def test_add_folder_pattern(self, mock_askdirectory):
        ...
        with home_dialog() as dlg:
            ...
            dlg.pump_events()
            # Then the pattern is added in the widget.
            self.assertEqual(1, len(dlg.backup.patterns))
            self.assertEqual(
                len(dlg.backup.patterns),
                len(dlg.patterns_view.scrolled_frame.interior.winfo_children()[0].winfo_children()),
            )
    ...
    def test_remove_pattern(self):
        ...
        with home_dialog() as dlg:
            ...
            with dlg.backup.patterns as t:
                t.clear()
                t.expand([item1, item2, item3, item4])
            dlg.patterns_view.data['patterns'] = dlg.backup.patterns
            self.assertEqual(4, len(dlg.backup.patterns))
            # When removing a pattern
            dlg.patterns_view.remove_pattern(item1)
            dlg.pump_events()
            # Then the pattern is added in the widget.
            self.assertEqual(3, len(dlg.backup.patterns))
            self.assertEqual(
                len(dlg.backup.patterns),
                len(dlg.patterns_view.scrolled_frame.interior.winfo_children()[0].winfo_children()),
            )
    ...
    def test_toggle_pattern(self):
        ...
        with home_dialog() as dlg:
            ...
            dlg.backup.patterns.append(item)
            dlg.patterns_view.data['patterns'] = dlg.backup.patterns
            self.assertEqual(1, len(dlg.backup.patterns))
            ...
            # Then the pattern is added in the widget.
            self.assertEqual(1, len(dlg.backup.patterns))
            self.assertFalse(dlg.backup.patterns[0].include)
