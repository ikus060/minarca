...
class TestMainParseArgs(unittest.TestCase):
    ...
    @mock.patch('minarca_client.main.MainDialog')
    ...
    def test_args_restore(self, mock_restore):
        ...
        mock_restore.assert_called_once_with(restore_time=None, force=False, pattern=['./test'], limit=limit(None))
    ...
    def test_args_forget(self, mock_forget):
        main.main(['forget'])
        mock_forget.assert_called_once_with(limit=limit(None))
    ...
    def test_restore(self, mock_backup):
        ...
        instance.restore.assert_called_once_with(restore_time=None, patterns=[Pattern(True, MATCH('*/test'), None)])
