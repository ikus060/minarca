...
class BackupInstance:
    ...
    async def _run_hooks(self, command, ignore_errors, log_file):
        ...
        if exit_code == 0:
            log_file.write(b'command completed successfully with exit code: %d' % exit_code)
            log_file.flush()
        elif ignore_errors:
            log_file.write(b'command failed with exit code: %d, but continuing execution' % exit_code)
            log_file.flush()
        else:
            log_file.write(b'command failed with exit code: %d' % exit_code)
                ...
                async with UpdateStatus(instance=self):
                    with open(self.backup_log_file, 'wb') as log_file:
                        ...
                        await self._run_hooks(
                            ...
                            ignore_errors=self.settings.ignore_hook_errors,
                            log_file=log_file,
                        )
