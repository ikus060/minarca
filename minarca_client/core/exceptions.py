...
class BackupError(Exception):
    """
    Generic error code for unknown or unhandled errors.
    """
...
class ConfigureBackupError(BackupError):
    """
    Raised during the configuration of a backup.
    """
...
class RdiffBackupException(BackupError):
    """
    Raised when backup encounters an unexpected exception.

    The internal backup process raised an unexpected exception.
...
class RdiffBackupExitError(BackupError):
    """
    Raised when the backup process return a non-zero exit status.
...
class ConnectException(BackupError):
    ...
    Connection to the remote server trought SSH protocl was refused. Make sure it's possible to
    ...
    Internet provider are blocking traffic when using default SSH port 22. You may beed to configure
...
class RemoteServerTruncatedHeader(BackupError):
    ...
    2. Ensure the connection is accepted by minarca-shell in `/var/log/minarca/shell.log`.
    """
